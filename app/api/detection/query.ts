...
export async function detectSolEtCultures({ ph, humidite, salinite }: { ph: number; humidite: number; salinite: number }) {
  // On cherche le sol le plus proche par similarité (distance euclidienne simple)
  const { data: sols, error: solsError } = await supabase.from("sols").select("*");
